    ...
    <div className="py-14 md:py-20" id="pricing-page">
      <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-3 font-display text-4xl font-bold text-stone-950 md:text-5xl">Plans from $29 USD/month</h1>
        ...
        <div className="mx-auto mt-10 max-w-3xl rounded-2xl border border-stone-200 bg-white p-5 sm:p-7">
          <fieldset><legend className="text-sm font-bold">View prices in</legend><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{([['USD','$','United States'],['GBP','£','United Kingdom'],['CAD','C$','Canada'],['AUD','A$','Australia']] as const).map(([code,symbol,region]) => <button key={code} type="button" aria-pressed={currency === code} onClick={() => setCurrency(code)} className={`relative min-h-16 rounded-xl border p-3 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700 ${currency === code ? "border-emerald-700 bg-emerald-50 text-emerald-950" : "border-stone-200 bg-white text-stone-700 hover:border-stone-300"}`}><span className="flex items-center justify-between"><strong>{code}</strong><span className="rounded-full bg-stone-100 px-2 py-0.5 text-xs font-bold">{symbol}</span></span><span className="mt-1 block truncate text-xs">{region}</span>{currency === code && <span aria-hidden="true" className="absolute right-2 top-2 text-emerald-700">✓</span>}</button>)}</div></fieldset>

...
export const LearningJourney: React.FC = () => (
  <section className="py-9 sm:py-12" aria-labelledby="journey-title"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div className="relative mx-auto mt-7 max-w-6xl"><div className="absolute bottom-9 left-[19px] top-9 w-0.5 bg-emerald-200 lg:bottom-auto lg:left-[10%] lg:right-[10%] lg:top-7 lg:h-0.5 lg:w-auto" aria-hidden="true" />
      <ol className="relative grid gap-3 lg:grid-cols-5 lg:gap-4">{milestones.map((milestone, index) => <li key={milestone.title} className="relative flex gap-3 rounded-xl border border-stone-200 bg-white p-3 lg:min-h-44 lg:flex-col lg:items-center lg:px-3 lg:pt-3 lg:text-center"><span className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-800 text-white ring-4 ring-stone-50"><Icon name={milestone.icon} size={17} /></span><div><p className="text-[10px] font-bold text-emerald-800">Step {index + 1}</p><h3 className="mt-1 text-sm font-bold text-stone-800">{milestone.title}</h3><p className="mt-1 text-xs leading-5 text-stone-600">{milestone.description}</p></div>{index === milestones.length - 1 && <span className="mt-auto text-[10px] font-bold text-amber-800">Advanced options</span>}</li>)}</ol>

...
export const MobileTrialBar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (heroCta) heroObserver.observe(heroCta); else setHeroCtaVisible(false);
    ...
    const onMenu = (event: Event) => setMenuOpen(Boolean((event as CustomEvent<{ open: boolean }>).detail?.open));
    window.addEventListener("mobile-menu-visibility", onMenu);
    return () => { heroObserver.disconnect(); footerObserver.disconnect(); window.removeEventListener("program-finder-visibility", onFinder); window.removeEventListener("mobile-menu-visibility", onMenu); };
  }, [dismissed, path]);
  ...
  if (dismissed || path.startsWith("/free-trial") || heroCtaVisible || footerVisible || finderOpen || menuOpen) return null;
  return <>
